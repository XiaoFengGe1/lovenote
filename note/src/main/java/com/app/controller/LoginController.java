...
public class LoginController {
	...
	public String note(@RequestParam("id") String noteid, HttpSession session,HttpServletRequest request) {
	...
	public String get() {
		if(timer!=null){
		    ...
		    timer.schedule(new TimerTask() {
				...
				public void run() {
					...
					emailData = "访问IP数:"+map.get("id")+"全网搜点击数:"+map.get("searchnum")+"首页点击数:"+map.get("indexnum")+"<br>所有用户"+map.get("users");
		...
		Map<String,String> map = recordService.getData();
		emailData = "访问IP数:"+map.get("id")+"全网搜点击数:"+map.get("searchnum")+"首页点击数:"+map.get("indexnum")+"所有用户:"+map.get("users");
		return emailData;
	}
	/*//微信公众号请求地址....因公众号审核问题，暂时弃用。
	@RequestMapping(value = "/weixinlovenote", method = RequestMethod.GET)
	@ResponseBody
	public String weixin(HttpServletResponse resp, HttpServletRequest request) {
		   // 微信加密签名  
        ...
        if (WinxinSignatureTool.checkSignature(signature, timestamp, nonce)) {  
        	return echostr;  
        }  
		return "";
	}*/
