...
$(document).ready(function(){
	 $.ajax({
	        ...
	        success:function(data,textStatus){
	        	...
	        	if(data.data["part"]==0){
	        		 $("#likeNote").html('<span class="glyphicon glyphicon-heart"></span>已赞');
	         ...
	         error:function(XMLHttpRequest, textStatus, errorThrown){
	        	 layer.close(load);
	        	 layer.tips('服务器错误，请稍后再试','#searchBtn',{tips: [1, '#3595CC'],time: 1000});
	         }
	    });
	 getReview();
});
...
$("#editNote").on("click",function(){
	...
	location.href="/android/write";
});

$("#sendreview").on("click",function(){
	if(buttonFlag ==0){return;}buttonFlag =0;
	if(sessionStorage.isLogin==0){
		layer.tips('请登录','#sendreview',{tips: [1, '#3595CC'],time: 2000});
		setButtonEnabled();
		return
	}
	if($("#reviewContent").val().trim()==""){
		layer.tips('请输入评论','#sendreview',{tips: [1, '#3595CC'],time: 2000});
		setButtonEnabled();
		return
	}
	 $.ajax({
	        ...
	        url: "/addreview",
	        async:true,
	        data: {content:$("#reviewContent").val().trim()},
	        dataType: "json",
	        success:function(data,textStatus){
	        	if(data.status=="true"){
	        		 layer.tips('评论成功，正在刷新页面','#sendreview',{tips: [1, '#3595CC'],time: 1000});
	        		 location.href="/android/note?id="+data.noteid;
	        		 return;
	        	}else{
	        		 layer.tips('服务器错误，请稍后再试','#sendreview',{tips: [1, '#3595CC'],time: 1000});
	        	}
	        	setButtonEnabled();
	               },
	         error:function(XMLHttpRequest, textStatus, errorThrown){
	...
	layer.confirm('确定删除吗？', {
	    btn: ['确定','取消'] //按钮
		 ...
		 $.ajax({
		        ...
		        success:function(data,textStatus){
		        	if(data.status=="true"){
		        		 layer.tips('删除成功，正在跳转首页','#deleteNote',{tips: [1, '#3595CC'],time: 1000});
		        		 location.href="/android/index";
		        		 return;
		        	}else{
		        		 layer.tips('服务器错误，请稍后再试','#deleteNote',{tips: [1, '#3595CC'],time: 1000});
		        	}
	...
	if(buttonFlag ==0){return;}buttonFlag =0;
	 $.ajax({
		        ...
		        success:function(data,textStatus){
		        	if(data.status=="true"){
		        		 $("#likeNote").html('<span class="glyphicon glyphicon-heart"></span>已赞');
		        		 layer.tips('ok','#likeNote',{tips: [1, '#3595CC'],time: 1000});
		        	}else{
		        		 layer.tips('服务器错误，请稍后再试','#likeNote',{tips: [1, '#3595CC'],time: 1000});
		        	}
		        	setButtonEnabled();
		               },
		         error:function(XMLHttpRequest, textStatus, errorThrown){
